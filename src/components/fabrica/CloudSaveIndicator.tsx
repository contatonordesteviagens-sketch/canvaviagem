...
export const CloudSaveIndicator = () => {
  ...
  if (syncStatus === "saving") {
    ...
    label = time ? `Salvo na nuvem · ${time}` : "Salvo na nuvem";
    cls = "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border-emerald-500/30";
  } else if (syncStatus === "error") {
    icon = <CloudOff className="w-3.5 h-3.5" />;
    label = "Falha ao salvar — tentando de novo";
    cls = "bg-red-500/10 text-red-700 dark:text-red-300 border-red-500/30";
  }

  return (
    <div
      title={lastSyncedAt ? `Última confirmação do servidor: ${lastSyncedAt.toLocaleString("pt-BR")}` : "Ainda não houve confirmação do servidor nesta sessão"}
