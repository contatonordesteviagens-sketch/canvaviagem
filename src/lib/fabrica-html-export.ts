...
/* WHATSAPP FLUTUANTE */
.wpp-float{position:fixed!important;bottom:max(20px,env(safe-area-inset-bottom));right:max(20px,env(safe-area-inset-right));left:auto!important;background:#25D366;color:#fff;width:58px;height:58px;border-radius:50%;display:flex;align-items:center;justify-content:center;box-shadow:0 10px 30px rgba(37,211,102,.5);z-index:9999;transition:transform .2s;isolation:isolate}
.wpp-float svg{display:block;width:31px;height:31px;fill:currentColor}
.wpp-float:hover{transform:scale(1.08)}
@media (max-width: 640px){.wpp-float{width:54px;height:54px;bottom:max(16px,env(safe-area-inset-bottom));right:max(16px,env(safe-area-inset-right))}.wpp-float svg{width:29px;height:29px}}
