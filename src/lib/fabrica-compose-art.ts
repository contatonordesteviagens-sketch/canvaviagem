...
function drawMonoIcon(
  ...
  switch (kind) {
    case "bus": {
      ...
      // Corpo do Ã´nibus (contorno)
      roundRect(ctx, x + s * 0.15, y + s * 0.2, s * 0.7, s * 0.55, s * 0.08);
      ctx.stroke();
      // PÃ¡ra-brisas (linha horizontal dividindo o Ã´nibus)
      ctx.beginPath();
      ctx.moveTo(x + s * 0.15, y + s * 0.48);
      ctx.lineTo(x + s * 0.85, y + s * 0.48);
      ctx.stroke();
      // Grade central vertical no pÃ¡ra-brisas
      ctx.beginPath();
      ctx.moveTo(cx, y + s * 0.2);
      ctx.lineTo(cx, y + s * 0.48);
      ctx.stroke();
      // FarÃ³is (pequenos cÃ­rculos contornados)
    ...
    case "hotel": {
      ctx.lineWidth = s * 0.09;
      // Fachada externa do prÃ©dio (contorno)
      roundRect(ctx, x + s * 0.2, y + s * 0.2, s * 0.6, s * 0.7, s * 0.06);
      ctx.stroke();
      // Porta arqueada na base do prÃ©dio
      ctx.beginPath();
    ...
    case "camera": {
      ...
      // Corpo principal da cÃ¢mera (contorno)
      ...
      // BotÃ£o de disparo do flash (preenchido)
...
/**
 * Desenha o glifo do Pix (4 losangos formando um padrÃ£o de "X"/diamante).
...
const panelBottom = RULES.PANEL_BOTTOM;
  ...
  const renderSafeSquareOffer = async () => {
    ...
    if (variant === 5) {
      ...
      const iconSizeV5 = 44;
      const iconGapV5 = 12;
      const daysW = daysTextV5 ? ctx.measureText(daysTextV5.toUpperCase()).width : 0;
      const iconStartX = leftPadV5 + (daysW > 0 ? daysW + 20 : 0);
      const iconStartY = cardY + 155 - 20 - (iconSizeV5 / 2); // Vertically align with daysTextV5
      ...
      const priceBaseY = cardY + basePriceOffset - 5; // Desceu para nao sobrepor os icones
      ...
      // Pix/Promo Badge lateral (Abaixo do price block)
        if (showPixBanner) {
          ...
          let textPart = rawPixLabel;
          let hasPixIcon = false;
          if (textPart.endsWith(" PIX")) {
             textPart = textPart.substring(0, textPart.length - 4).trim();
             hasPixIcon = true;
          } else if (textPart === "PIX") {
             textPart = "";
             hasPixIcon = true;
          }

          ctx.font = "900 16px Inter, Arial, sans-serif";
          const textW = textPart ? ctx.measureText(textPart).width : 0;
          const pixIconSize = 20;
          const pixGap = 8;
          const wordPixW = ctx.measureText("PIX").width;
          
          let contentW = textW;
          if (hasPixIcon) {
              contentW += (textW > 0 ? pixGap : 0) + pixIconSize + 4 + wordPixW;
          }
          
          const pixPadX = 14;
          const pixW = contentW + pixPadX * 2;
          const pixH = 32;
          const pixY = priceBaseY + (bottomTxt ? 50 : 25); // increased gap slightly
          ...
          let startX = rightX - pixW + pixPadX;
          
          if (textPart) {
             ctx.textAlign = "left";
             ctx.fillText(textPart, startX, pixY + pixH / 2 + 1);
             startX += textW + pixGap;
          }
          
          if (hasPixIcon) {
             const px = startX;
             const py = pixY + pixH / 2 - pixIconSize / 2;
             ...
             ctx.beginPath();
             ctx.moveTo(0, -s/2.2); ctx.lineTo(s/2.2, 0); ctx.lineTo(0, s/2.2); ctx.lineTo(-s/2.2, 0);
             ctx.closePath();
             ctx.fill();
             ctx.globalCompositeOperation = "destination-out";
             ctx.fillStyle = "rgba(0,0,0,1)";
             const gap = s * 0.15;
             ctx.fillRect(-s, -gap/2, s*2, gap);
             ctx.fillRect(-gap/2, -s, gap, s*2);
             ctx.globalCompositeOperation = "source-over";
             
             ctx.fillStyle = contrastOn(v5Secondary);
             ctx.beginPath();
             const ins = s * 0.18;
             ctx.moveTo(0, -ins); ctx.lineTo(ins, 0); ctx.lineTo(0, ins); ctx.lineTo(-ins, 0);
             ctx.closePath();
             ctx.fill();
             ctx.restore();
             
             ctx.textAlign = "left";
             ctx.fillText("PIX", px + pixIconSize + 4, pixY + pixH / 2 + 1);
          }
          ctx.textBaseline = "alphabetic";
        }
