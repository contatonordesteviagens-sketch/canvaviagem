...
export const Phase4LandingBuilderES = ({ onBack, onNext }: { onBack: () => void; onNext: () => void }) => {
  ...
  const previewHTML = user?.id
    ...
    : `<!doctype html><html><head><meta charset="utf-8"><style>*{box-sizing:border-box}body{margin:0;font-family:Arial;background:#fff;color:#111}.hero{height:440px;padding:60px 8%;background:linear-gradient(135deg,${state.primaryColor || "#7c3aed"},${state.secondaryColor || "#2563eb"});color:white}.tag{font-size:12px;font-weight:800;letter-spacing:2px}.title{font-size:48px;font-weight:900;max-width:650px;margin:22px 0}.cta{display:inline-block;padding:16px 24px;border-radius:14px;background:#fff;color:#111;font-weight:900}.cards{display:grid;grid-template-columns:repeat(3,1fr);gap:18px;padding:38px 8%}.card{height:210px;border-radius:22px;background:linear-gradient(145deg,#e5e7eb,#f8fafc);filter:blur(4px)}.lock{text-align:center;padding:28px;font-size:20px;font-weight:900}</style></head><body><section class="hero"><div class="tag">VISTA PREVIA DEL SITIO DE TU AGENCIA</div><div class="title">${String(state.agencyName || "Tu Agencia de Viajes").replace(/[<>&"']/g, "")}</div><div class="cta">Solicitar presupuesto</div></section><section class="cards"><div class="card"></div><div class="card"></div><div class="card"></div></section><div class="lock">Suscríbete al Plan Elite para montar y ver el sitio completo.</div></body></html>`;
