...
const createGuestPreviewArt = async (art: GeneratedArt): Promise<GeneratedArt> => {
  try {
    ...
    context.fillText("VISTA PREVIA • CREA TU CUENTA", 0, 0);
    context.restore();
    return { ...art, url: canvas.toDataURL("image/jpeg", 0.62) };
  } catch {
    const fallback = `<svg xmlns="http://www.w3.org/2000/svg" width="420" height="525"><rect width="100%" height="100%" fill="#111827"/><text x="50%" y="47%" fill="white" text-anchor="middle" font-family="Arial" font-size="24" font-weight="700">Tu anuncio está listo</text><text x="50%" y="54%" fill="#a78bfa" text-anchor="middle" font-family="Arial" font-size="17">Crea tu cuenta para verlo</text></svg>`;
...
const DEFAULT_COLORS_OFERTA = { primary: "#F5F906", secondary: "#FCD34D" };
...
type Currency = "BRL" | "USD1" | "USD2" | "USD3" | "EUR" | "ARS" | "GBP" | "COP" | "CLP" | "MXN" | "PEN";
const CURRENCY_PRESETS: { id: Currency; symbol: string; label: string; locale: string }[] = [
  { id: "USD1", symbol: "$", label: "Dólar ($)", locale: "en-US" },
  { id: "USD2", symbol: "US$", label: "Dólar (US$)", locale: "en-US" },
  { id: "USD3", symbol: "USD", label: "Dólar (USD)", locale: "en-US" },
  { id: "ARS", symbol: "ARS", label: "Peso ARS", locale: "es-AR" },
  { id: "CLP", symbol: "CLP", label: "Peso CLP", locale: "es-CL" },
  { id: "COP", symbol: "COP", label: "Peso COP", locale: "es-CO" },
  { id: "MXN", symbol: "MXN", label: "Peso MXN", locale: "es-MX" },
  { id: "PEN", symbol: "S/", label: "Sol PEN", locale: "es-PE" },
  { id: "BRL", symbol: "R$", label: "Real (R$)", locale: "pt-BR" },
  { id: "EUR", symbol: "€", label: "Euro (€)", locale: "es-ES" },
];
...
const buildTitleVariations = (template: string, destination: string): string[] => {
  ...
  // 🛡️ CORRECCIÓN CRÍTICA (Bloqueo de fuga de título):
  // El usuario exige control total. El título que elige o ingresa 
  // DEBE ser el título renderizado en todas las variantes.
  // La variación automática ha sido abolida para garantizar previsibilidad comercial.
...
export const Phase3ArtFactoryES = ({ onNext, onBack, initialMode = "ad", lockMode = false, onSkipToSite }: Props) => {
  ...
  const POPULAR_PHOTO_DESTINATIONS = [
    "Cancún", "Punta Cana", "Santiago", "Bariloche", "Machu Picchu",
    "Cartagena", "Buenos Aires", "San Pedro de Atacama", "Uyuni", "Cusco",
    "Mendoza", "Rio de Janeiro", "Bogotá", "Ciudad de México", "Galápagos",
  ];
  ...
  const generate = async (forceVariation?: number, accumulate: boolean = false) => {
    ...
    try {
      ...
      const finishCycle = (amount: number) => {
        ...
        track("ad_preview_generated", { amount, mode: genMode, format, locale: "es" });
      };
      if (!user) {
        ...
        finishCycle(1);
        toast.success("Tu vista previa está lista. Crea tu cuenta para generar el anuncio final.");
  ...
  return (
    <div className={`${creativeMode === "carousel" ? "w-full max-w-[1440px]" : "max-w-3xl"} mx-auto space-y-6`}>
                      ...
                      {isAdPreviewLocked ? (
                        ...
                        >
                          ...
                          <span className="text-xs font-black">
                            {tier === "guest"
                              ? "Crea tu cuenta para desbloquear"
      ...
      <FabricaPaywallDialog
        ...
        description="Tu proyecto sigue guardado. Activa Elite para descargar nuevos anuncios sin límite."
      />
    </div>
  );
};
