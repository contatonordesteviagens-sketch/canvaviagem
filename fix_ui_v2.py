...
# 1. Update Vercel Logic (Delete old project)
vercel_old = """      // Envia deploy para o Vercel usando API v13"""
vercel_new = """      const domain = `${cleanSubdomain}.vercel.app`;
...
# 2. Update Canva Viagem Logic (Use .webp and delete old)
canva_old = """    try {
...
# Also update the user.id.html to .webp
canva_user_old = """      await supabase.storage
        .from("thumbnails")
        .upload(`sites/${user.id}.html`, blob, {"""
canva_user_new = """      await supabase.storage
        .from("thumbnails")
        .upload(`sites/${user.id}.webp`, blob, {"""
content = content.replace(canva_user_old, canva_user_new)


# 3. Fix UI: Pixel inputs move down, all colors white/gray, duplicate buttons.
ui_old = """            <div className="grid grid-cols-2 gap-4 mt-4">
              <div>
            ...
            <button
              ...
              className="w-full mt-4 py-3.5 px-4 rounded-xl font-bold text-white flex items-center justify-center gap-2 hover:brightness-110 disabled:brightness-50 disabled:cursor-not-allowed transition-all text-sm"
              style={{ 
        ...
        <div className="my-4 p-6 rounded-2xl border-2 border-cyan-400/30 bg-cyan-400/[0.05] backdrop-blur-xl text-left">
          <div className="flex items-start justify-between gap-4 mb-4">
            <div>
              ...
              <p className="text-xs text-white/60 mt-1 leading-relaxed">
                Gera um link seguro instantâneo no seu subdomínio sem precisar de Vercel.
              </p>
            </div>
          ...
          {state.siteContent.canvaViagemUrl && (
            <div className="mt-3 mb-4 p-4 rounded-xl bg-cyan-400/10 border border-cyan-300/25 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div>
          ...
          <button
            ...
            className="w-full mt-4 py-3.5 px-4 rounded-xl font-bold text-black flex items-center justify-center gap-2 hover:brightness-110 disabled:brightness-50 disabled:cursor-not-allowed transition-all text-sm bg-cyan-400"
          >
            {isCanvaViagemPublishing ? (
              ...
              <>
                Publicar Link Rápido
              </>
            )}
          </button>
        </div>"""

ui_new = """            {showVercelConfig && (
...
# Standardize Emerald Vercel Success block
...
# Insert Pixel tracking at the bottom
...
print("ALL CHANGES APPLIED SUCCESSFULLY TO 64e29e1!")
